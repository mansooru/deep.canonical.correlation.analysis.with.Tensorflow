...
# load DataSet
data=h5py.File('./02.data/real.dataset.mat')
train_X1 = np.transpose(np.mat(data['gene']))
train_X2 = np.transpose(np.mat(data['spect']))

n_snp_features = 3365
n_img_features = 90
n_inter_features = 2500
...
cost = (alpha)*tf.reduce_mean(tf.square(genBA-X2))- (1-alpha)*corr + 1*tf.norm(Fs_img,ord=1)# + 0.05*tf.matmul(tf.matmul(Fs_img,PY),tf.transpose(Fs_img))
cost1 = (alpha)*tf.reduce_mean(tf.square(genAB-X1)) - (1-alpha)*corr  + 1*tf.norm(Fs_snp,ord=1)# + 0.05*tf.matmul(tf.matmul(Fs_snp,PX),tf.transpose(Fs_snp))
...
with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
    ...
    optimizer1 = tf.train.AdagradOptimizer(0.8).minimize(cost1)
...
for epoch in range(n_epochs):
    ...
    for current_batch_index in range(0,train_X1.shape[0],batch_size):
        ...
        cor, test_loss = sess.run([corr, cost], feed_dict={X1:train_X1, X2:train_X2})
